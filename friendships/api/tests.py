...
class FriendshipApiTests(TestCase):
    ...
    def test_follow(self):
        ...
        self.assertEqual(response.data['user']['username'], 'linghu')
        # 重复 follow 失败
        response = self.dongxie_client.post(url)
        self.assertEqual(response.status_code, 400)
        # 反向关注会创建新的数据
        before_count = FriendshipsService.get_following_count(self.linghu.id)
        response = self.linghu_client.post(FOLLOW_URL.format(self.dongxie.id))
        self.assertEqual(response.status_code, 201)
        after_count = FriendshipsService.get_following_count(self.linghu.id)
    ...
    def test_unfollow(self):
        ...
        before_count = FriendshipsService.get_following_count(self.dongxie.id)
        response = self.dongxie_client.post(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['deleted'], 1)
        after_count = FriendshipsService.get_following_count(self.dongxie.id)
        self.assertEqual(after_count, before_count - 1)
